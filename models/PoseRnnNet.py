...
class PoseExpNet(nn.Module):
    ...
    def __init__(self, nb_ref_imgs=2, output_exp=False):    ##input 3 images---t-1, t, t+1;  
        ...
        self.lstm1= nn.LSTMCell(conv_planes[6], 128)
        self.pose_pred= nn.LSTMCell(128,  6*self.nb_ref_imgs)
          
        
        self.pose_pred = nn.Conv2d(conv_planes[6], 6*self.nb_ref_imgs, kernel_size=1, padding=0)    ###output 12 parameters-------6 for Tt->t-1, 6 for Tt->t+1
    ...
    def forward(self, target_image, ref_imgs):
        ...
        out_lstm1=self.lstm1(out_conv7)
        pose = self.pose_pred(out_lstm1)
        ...
        if self.training:
            return [exp_mask1, exp_mask2, exp_mask3, exp_mask4], pose            ###if training, return pose!!!
        else:
            return  pose
